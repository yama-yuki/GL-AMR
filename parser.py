...
from GL_predict import predict_label
...
class Parser(object):
    ...
    def get_best_act(self,scores,actions,GL_status,RE_status,cur_word='None',mismatch='None',noun_obj='None'):
        best_label_index = None
        best_act_ind = np.argmax(map(np.amax,scores))
        best_act = actions[best_act_ind]

        ASP=['begin','began','begun','beginning','start','started','starting','finish','finished','finishing','end','ended','ending']

        GL = 'None'
        print('GL_status: '+GL_status)
        print('RE_status: '+RE_status)
        
        if cur_word == noun_obj:
            if {'type': 8} in actions:
                print('type: 8 in possible actions')
                if mismatch != 'None':
                    if GL_status == 'not-applied':
                        print('mismatch_detected: '+str(mismatch))
                        best_act = {'type': 8}
                        best_act_ind = actions.index({'type': 8})
                        GL = 'COMP'
                        GL_status='applied'

        if GL_status == 'TC-applied':
            if cur_word.lower() in ASP:
                if RE_status == 'not-applied':
                    count=0
                    for a in actions:
                        if a['type'] == 5 and a['parent_to_add'] == 'ap0':
                            print('type: 5 in possible actions')
                            best_act = actions[count]
                            best_act_ind = count
                            RE_status = 'applied'
                            break
                        count+=1        

        act_num = best_act['type']
        #print(act_num)
        print('best_act')
        print(best_act)
        if best_act['type'] in ACTION_WITH_EDGE or best_act['type'] in ACTION_WITH_TAG:
            best_label_index = scores[best_act_ind].argmax()
        return best_act_ind, best_label_index, GL, GL_status, RE_status, act_num
    ...
    def parse_corpus_test(self, instances, EVAL=False):
        ...
        if EVAL:
            ...
            for i,inst in enumerate(instances,1):
                per_start_time = time.time()
                step,state = self.parse(inst,i,train=False)
                per_parse_time = round(time.time()-per_start_time,3)
                parsed_amr.append(GraphState.get_parsed_amr(state.A))
    ...
    def parse(self,instance,num='None',train=True): 
        ...
        if num != 'None':##
            SN = num-1
            print('sentence_number: '+str(SN))
            with open('pickle_GO','rb') as pGO:
                GO = pickle.load(pGO)
            with open('pickle_SL','rb') as pSL:
                SL = pickle.load(pSL)
        ###################
        sent = SL[SN]
        mismatch = GO[SN][0]
        noun_obj = GO[SN][1]
        '''
        GCOMP = ['cakes','curry','items','bread','meal','fajitas','boudin','omelets','food']
        if noun_obj in GCOMP:
            mismatch = 'CC'
        else: mismatch = 'None'
        '''
        GL_status = 'not-applied'
        RE_status = 'not-applied'
        ###################
        ...
        while not state.is_terminal():
            ...
            if len(actions) == 1:
                ...
                if train:
                    ...
                    added_node = 'None'
                    if state.idx != 'st':#
                        tag = state.get_current_node()#
                        print(vars(tag))
                        cur_word = tag.words[0]#
                        if state.idx == 'ap':
                            added_node = 'True'

                    print('STEP: '+str(step))
                    print('checking word: '+cur_word)
                    best_act_ind, best_label_index, GL, status, restatus, act_num = self.get_best_act(scores,actions,GL_status,RE_status,cur_word,mismatch,noun_obj)#,argset)
                    
                    if GL == 'COMP':
                        print('GENERATIVE OPERATION')
                    best_act = actions[best_act_ind]
                    best_actions.append(best_act)#
                    print('best_act: '+str(best_act))
                    best_label = Parser.get_index_label(best_act,best_label_index)
                    if GL == 'COMP' and mismatch == 'CC':
                        print('GENERATIVE OPERATION: CO-COMPOSITION')
                        best_label = 'exist-01'
                        GL_status = 'CC-applied'
                        
                    elif GL == 'COMP' and mismatch == 'TC':
                        print('GENERATIVE OPERATION: TYPE COERCION')
                        print('LOADING... MASKED LANGUAGE MODEL')
                        best_label = predict_label(sent)
                        GL_status = 'TC-applied'

                    elif GL_status == 'TC-applied' and act_num == 5:
                        print('REENTRANCE')
                        best_label = 'ARG0'
                        RE_status = 'RE-applied'
            ...
            pre_state = state
            print('best_act+label')
            print(act_to_apply)
            
            print('Before: '+str(step))
            print(vars(pre_state))#
